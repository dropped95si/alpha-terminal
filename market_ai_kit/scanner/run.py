...
def analyze_ticker(ticker, cfg):
    ...
    # 2. Statistical Base Prob
    prob_engine = ProbabilityEngine()
    base_result = prob_engine.calculate_score(latest)
    
    # 3. Whale Validation
    whale_sys = WhaleValidator()
    whale = whale_sys.validate_whale_flow(latest)
    whale_boosted = apply_whale_boost(base_result['probability'], whale)
    
    # 4. Credibility & Final
    cred_sys = AdaptiveWeightingSystem()
    cred_context = {**latest, 'whale_conviction': whale['conviction'], 'base_probability': base_result['probability']}
    cred = cred_sys.assess_credibility(cred_context)
    ...
    risk = risk_engine.calculate_position(
        account_size=risk_cfg.get('account_size', 100000), 
        entry=latest['close'], 
